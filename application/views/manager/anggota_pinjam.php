...
$this->fpdf->Cell(19,0.5,'LAPORAN ANGGOTA YANG MEMINJAM',0,0,'C');
$this->fpdf->Ln(1);
$this->fpdf->Line(1,3.5,20,3.5);
$this->fpdf->Line(1,3.55,20,3.55);
$this->fpdf->Ln();

$count=count($anggotapinjam);
 if($count>0)
 {
$this->fpdf->SetFont('helvetica','',8);
...
$this->fpdf->Cell(10, 1, 'Nama Anggota', 1, '0', 'C');
$this->fpdf->Cell(3, 1, 'Banyak Meminjam', 1, '0', 'C');
 $no=1;
 foreach($anggotapinjam as $data)
{
    $this->fpdf->Ln();
    $this->fpdf->SetFont('Times','',7);
    $this->fpdf->Cell(0.5, 0.7, $no, 1, '0', 'C');
    $this->fpdf->Cell(10, 0.7, $data->nama_anggota, 1, '0', 'L');
...
{	
	...
	$this->fpdf->Cell(13,0.5,'LAPORAN PEMINJAMAN KOSONG',0,0,'C');
}
$this->fpdf->SetMargins(1,1,1);
$this->fpdf->SetY(-3); 
